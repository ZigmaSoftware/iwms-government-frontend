...
import { municipalityApi, stateApi, districtApi } from "@/helpers/admin";
...
export default function MunicipalityForm() {
  const { showField, filterPayload } = useFieldVisibility("masters", "municipalities", MUNICIPALITY_FIELDS);
  ...
  useEffect(() => {
    ...
    municipalityApi.read(id).then((res: any) => {
      if (cancelled) return;
      setRecordData(res);
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!companyUniqueId) {
      Swal.fire("Error", "Company is required", "error");
      return;
    }
    if (!projectId) {
    ...
    try {
      if (isEdit && id) {
        await municipalityApi.update(id, payload);
        Swal.fire("Success", "Updated successfully", "success");
      } else {
        await municipalityApi.create(payload);

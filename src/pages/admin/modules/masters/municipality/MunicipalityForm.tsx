...
export default function MunicipalityForm() {
  ...
  useEffect(() => {
    if (!isEdit || !id) return;
    let cancelled = false;
    municipalityApi.read(id)
      .then((record: RecordRow) => {
        if (cancelled) return;
        setName(textOf(record, "municipality_name", "name", "state_name", "district_name", "area_type_name"));
      ...
      .catch(() => Swal.fire("Error", "Failed to load Municipality", "error"));
    return () => { cancelled = true; };
  }, [id, isEdit]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!name.trim()) {
      Swal.fire("Missing details", "Municipality Name is required", "warning");
      return;
    }
    ...
    setSubmitting(true);
    try {
      if (isEdit && id) await municipalityApi.update(id, payload);
      else await municipalityApi.create(payload);
      Swal.fire("Success", "Municipality saved successfully", "success");
      navigate(LIST_PATH);
    } catch (error: any) {
      Swal.fire("Error", String(error?.response?.data?.detail ?? error?.message ?? "Failed to save Municipality"), "error");
    } finally {
      setSubmitting(false);
    }
  };

...
const SORTABLE_FIELDS = new Set(["municipality_name", "is_active"]);
...
export default function MunicipalityListPage() {
  ...
  const loadRows = async (page: number, limit: number, search: string, ordering?: string) => {
    setIsLoading(true);
    try {
      const response = await municipalityApi.readAllwithPaginated(page, limit, {
        params: {
          ...(search ? { search } : {}),
          ...(ordering ? { ordering } : {}),
        },
  ...
  const onExportRequest = async () => toRecordList(await municipalityApi.readAllForExport());

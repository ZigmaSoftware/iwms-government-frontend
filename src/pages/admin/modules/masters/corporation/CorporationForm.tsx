...
export default function CorporationForm() {
  ...
  useEffect(() => {
    if (!isEdit || !id) return;
    let cancelled = false;
    corporationApi.read(id)
      .then((record: RecordRow) => {
        if (cancelled) return;
        setName(textOf(record, "corporation_name", "name", "state_name", "district_name", "area_type_name"));
      ...
      .catch(() => Swal.fire("Error", "Failed to load Corporation", "error"));
    return () => { cancelled = true; };
  }, [id, isEdit]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!name.trim()) {
      Swal.fire("Missing details", "Corporation Name is required", "warning");
      return;
    }
    ...
    setSubmitting(true);
    try {
      if (isEdit && id) await corporationApi.update(id, payload);
      else await corporationApi.create(payload);
      Swal.fire("Success", "Corporation saved successfully", "success");
      navigate(LIST_PATH);
    } catch (error: any) {
      Swal.fire("Error", String(error?.response?.data?.detail ?? error?.message ?? "Failed to save Corporation"), "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <ComponentCard title={isEdit ? "Edit Corporation" : "Add Corporation"}>

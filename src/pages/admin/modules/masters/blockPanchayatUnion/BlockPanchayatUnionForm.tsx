...
export default function BlockPanchayatUnionForm() {
  const { showField, filterPayload } = useFieldVisibility("masters", "block-panchayat-unions", BLOCK_FIELDS);
  ...
  const { listPath: LIST_PATH } = createCrudRoutePaths(encMasters, encBlockPanchayatUnions);
  ...
  useEffect(() => {
    ...
    blockPanchayatUnionApi.read(id).then((res: any) => {
      if (cancelled) return;
      setRecordData(res);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isEdit && id) {
        await blockPanchayatUnionApi.update(id, payload);
        Swal.fire("Success", "Updated successfully", "success");
      } else {
        await blockPanchayatUnionApi.create(payload);
  ...
  return (
    <ComponentCard title={isEdit ? "Edit Block / Panchayat Union" : "Add Block / Panchayat Union"}>

...
import type { TownPanchayatListRecord } from "./types";

const normalizeId = (value: unknown): string =>
  value === null || value === undefined ? "" : String(value).trim();

const TOWN_PANCHAYAT_COLUMN_FIELDS: Record<string, string[]> = {
...
export default function TownPanchayatListPage() {
  ...
  const { showColumn: showCol, filterPayload } = useFieldVisibility(
    "masters",
    "town-panchayats",
    TOWN_PANCHAYAT_COLUMN_FIELDS,
  );
  const [allTownPanchayats, setAllTownPanchayats] = useState<TownPanchayatListRecord[]>([]);
  ...
  const [filters, setFilters] = useState<DataTableFilterMeta>({
    ...
    town_panchayat_name: { value: null as string | null, matchMode: FilterMatchMode.STARTS_WITH },
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        const data = await townPanchayatApi.readAll();
        if (mounted) setAllTownPanchayats(data as TownPanchayatListRecord[]);
      } catch (error) {
  ...
  const data = ((): TownPanchayatListRecord[] => {
    if (isSuperAdmin && companies.length === 0) return [];
    if (!companyUniqueId && !isSuperAdmin) return [];
    return (Array.isArray(allTownPanchayats) ? allTownPanchayats : []).filter((row) => {
  ...
  const statusTemplate = (row: TownPanchayatListRecord) => {
    const updateStatus = async (value: boolean) => {
      const id = String(row.unique_id);
      setPendingStatusId(id);
      setIsUpdating(true);
      try {
        await townPanchayatApi.update(row.unique_id, filterPayload({ is_active: value }) as { is_active: boolean });
        setAllTownPanchayats((current) =>
          current.map((item) => item.unique_id === row.unique_id ? { ...item, is_active: value } : item)
        );
      } catch (error) {
        console.error("Failed to update town panchayat status", error);
      } finally {
  ...
  const indexTemplate = (_: TownPanchayatListRecord, { rowIndex }: { rowIndex: number }) => rowIndex + 1;

  return (
    <div className="p-3">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-1">Town Panchayat</h1>
          <p className="text-sm text-gray-500">Manage Town Panchayat records</p>
        </div>
      ...
      <DataTable
        ...
        stripedRows
        showGridlines
        emptyMessage="No town panchayats found."
        globalFilterFields={["town_panchayat_name", "state_name", "district_name", "company_name", "project_name"]}
        className="p-datatable-sm"
      >
        <Column header={t("common.s_no")} body={indexTemplate} style={{ width: "80px" }} />
        {showCol("town_panchayat_name") && (
          <Column field="town_panchayat_name" header="Town Panchayat" sortable filter showFilterMatchModes={false}
            body={(row: TownPanchayatListRecord) => cap(row.town_panchayat_name)} />
        )}
        {showCol("district_name") && (
          <Column field="district_name" header={t("common.district")} sortable filter showFilterMatchModes={false}
            body={(row: TownPanchayatListRecord) => cap(row.district_name)} />
        )}
        {showCol("state_name") && (
          <Column field="state_name" header={t("common.state")} sortable filter showFilterMatchModes={false}
            body={(row: TownPanchayatListRecord) => cap(row.state_name)} />
        )}
        {showCol("is_active") && (

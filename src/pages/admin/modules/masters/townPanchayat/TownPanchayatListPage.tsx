...
const toRecordList = (value: unknown): TownPanchayatListRecord[] => {
  if (Array.isArray(value)) return value as TownPanchayatListRecord[];
  if (value && typeof value === "object" && Array.isArray((value as { results?: unknown }).results)) {
    return (value as { results: TownPanchayatListRecord[] }).results;
  }
  return [];
};
...
const SORTABLE_FIELDS = new Set(["town_panchayat_name", "is_active"]);

const columns = [
  { field: "state_name", header: "State" },
  { field: "district_name", header: "District" },
  { field: "area_type_name", header: "Area Type" },
  { field: "town_panchayat_name", header: "Town Panchayat Name" },
...
export default function TownPanchayatListPage() {
  ...
  const [rows, setRows] = useState<TownPanchayatListRecord[]>([]);
  ...
  const loadRows = async (page: number, limit: number, search: string, ordering?: string) => {
    setIsLoading(true);
    try {
      const response = await townPanchayatApi.readAllwithPaginated(page, limit, {
        params: {
          ...(search ? { search } : {}),
          ...(ordering ? { ordering } : {}),
        },
  ...
  const onExportRequest = async () => toRecordList(await townPanchayatApi.readAllForExport());
  ...
  return (
    <div className="p-3">
      ...
      <DataTable
        ...
        stripedRows
        showGridlines
        emptyMessage="No Town Panchayat records found."
      ...
      >
        ...
        {columns.map((column) => (
          <Column
            ...
            body={(row: TownPanchayatListRecord) =>

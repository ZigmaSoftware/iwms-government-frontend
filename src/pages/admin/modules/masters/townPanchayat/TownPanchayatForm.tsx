...
export default function TownPanchayatForm() {
  ...
  useEffect(() => {
    if (!isEdit || !id) return;
    let cancelled = false;
    townPanchayatApi.read(id)
      .then((record: RecordRow) => {
        if (cancelled) return;
        setName(textOf(record, "town_panchayat_name", "name", "state_name", "district_name", "area_type_name"));
      ...
      .catch(() => Swal.fire("Error", "Failed to load Town Panchayat", "error"));
    return () => { cancelled = true; };
  }, [id, isEdit]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!name.trim()) {
      Swal.fire("Missing details", "Town Panchayat Name is required", "warning");
      return;
    }
    ...
    setSubmitting(true);
    try {
      if (isEdit && id) await townPanchayatApi.update(id, payload);
      else await townPanchayatApi.create(payload);
      Swal.fire("Success", "Town Panchayat saved successfully", "success");
      navigate(LIST_PATH);
    } catch (error: any) {
      Swal.fire("Error", String(error?.response?.data?.detail ?? error?.message ?? "Failed to save Town Panchayat"), "error");
    } finally {
      setSubmitting(false);
    }
  };

...
const toRecordList = (value: unknown): StateListRecord[] => {
  if (Array.isArray(value)) return value as StateListRecord[];
  if (value && typeof value === "object" && Array.isArray((value as { results?: unknown }).results)) {
    return (value as { results: StateListRecord[] }).results;
  }
  return [];
};
...
const SORTABLE_FIELDS = new Set(["state_name", "state_code"]);
...
export default function StateListPage() {
  ...
  const statusTemplate = (row: StateListRecord) => {
  ...
  const actionTemplate = (row: StateListRecord) => (
  ...
  return (
    <div className="p-3">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-1">State</h1>
          <p className="text-sm text-gray-500">Manage State records</p>
        </div>
        <Button
          label="Add State"
          icon="pi pi-plus"
